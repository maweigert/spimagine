...
import numpy as np
from spimagine.data_model import DataModel, SpimData, TiffData, NumpyData
...
def test_spimdata():
    d = SpimData(absPath("data/Drosophila_Single"))
...
def test_tiffdata():
    d = TiffData(absPath("data/HisStack_uint16_0000.tif"))
...
def test_data_sets():
    fNames = ["data/Drosophila_Single",
              "data/HisStack_uint16_0000.tif",
              "data/HisStack_uint8_0000.tif",
              # "test_data/meep.h5",
              "data/retina.czi"
          ]
    ...
    for fName in fNames:
        d = DataModel.fromPath(absPath(fName))
        print "----------DATASET LOADED-----------"
        print "---   %s  \t %s    ---"%(fName, d.size())
