...
def get_qt5_binaries():
    ...
    qt_sos = glob("/usr/local/lib/python2.7/site-packages/PyQt5/*.so")
    ...
    for fold, subs, files in os.walk(root):
        ...
        for fName in files:
            if reg.match(fName):
                ...
                res += [(name,os.path.join(fold,fName),attrib)]
    return res
...
a.datas += addAll("/Users/mweigert/python/gputools/gputools/fft/kernels")
